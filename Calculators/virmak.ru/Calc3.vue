  ...
  <div class="container">
    <div class="row">
        ...
        <div class="calc-row">
          <div class="column-1-3">
            ...
            <b-form-group
                class="metric--meter"
                label="Длина"
                label-for="NarStenPanelLength2"
            >
              <b-form-input
                  id="NarStenPanelLength2"
                  type="text"
                  :class="{ 'is-invalid': $v.NarStenPanelLength2.$error && errorMessages, 'animated shake': !$v.NarStenPanelLength2.required && errorMessages}"
                  v-model.trim.number="$v.NarStenPanelLength2.$model"
                  placeholder="Введите длину" />
              <div class="error"  v-if="!$v.NarStenPanelLength2.required && errorMessages">
                Обязательное поле
              </div>
              <div class="error"  v-if="!$v.NarStenPanelLength2.numeric && errorMessages">
                Поле должно состоять только из цифр
              </div>
            </b-form-group>
          </div>
          <div class="column-2-3">
            ...
            <b-form-group
                class="metric--meter"
                label="Ширина"
                label-for="NarStenPanelWidth2"
            >
              <b-form-input
                  id="NarStenPanelWidth2"
                  type="text"
                  :class="{ 'is-invalid': $v.NarStenPanelWidth2.$error && errorMessages, 'animated shake': !$v.NarStenPanelWidth2.required && errorMessages}"
                  v-model.trim.number="$v.NarStenPanelWidth2.$model"
                  placeholder="Введите ширину" />
              <div class="error"  v-if="!$v.NarStenPanelWidth2.required && errorMessages">
                Обязательное поле
              </div>
              <div class="error"  v-if="!$v.NarStenPanelWidth2.numeric && errorMessages">
                Поле должно состоять только из цифр
              </div>
            </b-form-group>
          </div>
      ...
      <div class="col-9 col-md-9 p-0 mb-5">
        ...
        <h4 class="title--4">{{ lorem.group4 }}</h4>
        <div class="calc-row">
          <div class="column-1-3">
            <!--Checkbox-->
            <b-form-checkbox
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        lorem: {
          name: 'Изовер Друхэтажный (с разными площадями пол мин.плита)',
          leftColumnTitle: 'Входные данные',
          rightColumnTitle: 'Результат',
          group1: 'Наружные стеновые панели (первый этаж)',
          group2: 'Наружные стеновые панели (второй этаж)',
          group3: 'Межкомнатные перегородки',
          group4: 'Кровельные панели'
        },
        NarStenPanelLength: null,
        NarStenPanelWidth: null,
        NarStenPanelLength2: null,
        NarStenPanelWidth2: null,
    ...
    methods: {
      ...
      totalCount() {
        ...
        if (!this.errorMessages) {
          const result = this.CountNapolPanel(3200) +
              this.CountNarStenPanel(3050) +
              this.CountNarStenPanel2(3050) +
      ...
      CountNapolPanel(costPanel) {
        ...
        let countPanelLength = Math.ceil(length / 0.6)
      ...
      CountNarStenPanel(costPanel) {
        ...
        let countPanelLength = Math.ceil((this.NarStenPanelLength - 0.156) / 0.6 * 2)
        let countPanelWidth = Math.ceil((this.NarStenPanelWidth - 0.156) / 0.6 * 2)
      ...
      //Наружные стеновые панели 2
      CountNarStenPanel2(costPanel) {
        const COST_PANEL = costPanel

        let countPanelLength = Math.ceil((this.NarStenPanelLength2 - 0.156) / 0.6 * 2)
        let countPanelWidth = Math.ceil((this.NarStenPanelWidth2 - 0.156) / 0.6 * 2)
        let allPanel = countPanelLength + countPanelWidth

        this.objCost.NarPanel2 = {allPan: allPanel}
        ...
        let flag = true
        tableResult.forEach(function(item) {
          if (item['title'] === 'Наружные стеновые панели 2') {
            flag = false
          }
        });
        if(flag) {
          this.tableResult.push({title: 'Наружные стеновые панели 2', value: result })
        } else {
          tableResult.forEach(function(item) {
            if (item['title'] === 'Наружные стеновые панели 2') {
              item['value'] = result
      ...
      CountBrus(obj, price) {
        ...
        const CountNarPanel2Col1 = Math.ceil(NarPanel2.allPan * 5.4 / 6 * 1.2)
        ...
        const CountNarPanel2Col2 = CountNarPanel2Col1 * 6
        const CountNapPanelCol2_CountNarPanelCol2 = CountNapPanelCol2 + CountNarPanelCol2
        const CountMegkPeregCol2 = CountMegkPeregCol1 * 4
        ...
        const NarPanelTotal2 = CountNarPanel2Col2 * price[1]
        const MegkPeregTotal = CountMegkPeregCol2 * price[2]
        const KrowPanelTotal = CountKrowPanelCol2 * price[3]
        //Итог
        const TotalCost = NapPanelTotal + NarPanelTotal + NarPanelTotal2 + MegkPeregTotal + KrowPanelTotal

        //log
        // console.log(`CountNapPanelCol1: ${CountNapPanelCol1}`)
        // console.log(`CountNarPanelCol1: ${CountNarPanelCol1}`)
        // console.log(`CountNarPanel2Col1: ${CountNarPanel2Col1} = ${NarPanel2.allPan} * 5.4 / 6 * 1.2`)
        // console.log(`CountMegkPeregCol1: ${CountMegkPeregCol1}`)
        // console.log(`CountKrowPanelCol1: ${CountKrowPanelCol1}`)
        //
        // console.log(`CountNapPanelCol2: ${CountNapPanelCol2}`)
        // console.log(`CountNarPanelCol2: ${CountNarPanelCol2}`)
        // console.log(`CountNarPanel2Col2: ${CountNarPanel2Col2}`)
        // console.log(`CountMegkPeregCol2: ${CountMegkPeregCol2}`)
        // console.log(`CountKrowPanelCol2: ${CountKrowPanelCol2}`)
      ...
      CountConsumables(obj, price) {
        const totalAllPanel = obj.NapPanel.allPan + obj.NarPanel.allPan + obj.NarPanel2.allPan + obj.MegkPereg.allPan + obj.KrowPanel.allPan
        //Пена монтажная
