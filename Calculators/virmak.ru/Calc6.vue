...
<script>
  ...
  export default {
    ...
    data() {
      return {
        lorem: {
          name: 'Изовер Мансардный (пол с мин.плитой)',
    ...
    methods: {
      ...
      CountNarStenPanel(costPanel) {
        ...
        let countPanelLength = Math.ceil((this.NarStenPanelLength - 0.156) / 0.6 * 2) + Math.ceil((this.NarStenPanelLength - 0.156) / 1.2)
        let countPanelWidth = Math.ceil((this.NarStenPanelWidth - 0.156) / 0.6 * 2) + Math.ceil((this.NarStenPanelWidth - 0.156) / 1.2)
        let allPanel = 0
        if (this.KrowPanel) {
          allPanel = countPanelLength + countPanelWidth + Math.ceil((0.5 * this.NarStenPanelWidth * (this.NarStenPanelWidth / 2) / 3.24)) * 2
        } else  {
          allPanel = countPanelLength + countPanelWidth
        }
